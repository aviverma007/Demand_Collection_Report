...
import {
  Building2, IndianRupee, TrendingUp, RotateCcw, BarChart3,
  PieChart as PieIcon, Activity, Wallet, LayoutGrid
} from 'lucide-react';
...
const TOWER_COLORS = [C.blue, C.brownLt, C.blueAcc, C.gold, C.green, C.rose];

/* ── exact tooltipStyle from App.jsx ─────────── */
const TT_STYLE = {
...
/* ── 3D KPI Card — cloned from KPI3D in App.jsx ─────────── */
  ...
  return (
    ...
    >
      ...
      <div className="kpi-icon-wrap" style={{
        ...
        border:`1.5px solid ${color}20`, color,
...
/* ── Glass Chart Card — cloned from GlassCard in App.jsx ─── */
function GlassCard({ title, icon, color='var(--blue)', children, wide, delay=0, style:extraStyle }) {
  const [hovered, setHovered] = useState(false);
  return (
    <div
    ...
    >
      <div className="chart-header">
        <span className="chart-icon-wrap" style={{
          color, background:`${color}10`, borderColor:`${color}18`,
...
export default function Dashboard() {
  ...
  return (
    <div className="dashboard">
      ...
      {/* Filters — exact glass style + reset button from App.jsx */}
      <div className="filters-bar">
        <div className="filter-group">
          ...
          <select className="filter-select" value={fp} onChange={e=>setFp(e.target.value)}>
            ...
            {s.projects.map(p=><option key={p} value={p}>{p}</option>)}
          </select>
        </div>
        <div className="filter-group">
      ...
      {/* KPI — 2 rows of 4 like App.jsx, with exact KPI3D component */}
      ...
      {tab==='overview' && (
        <div className="tab-content">
          <div className="charts-grid">
            ...
            {/* Monthly Demand vs Received — exact ComposedChart from App.jsx */}
                ...
                <ComposedChart data={data.monthly} margin={{top:18,right:22,left:5,bottom:20}}>
                  <defs>
                    <linearGradient id="demandBar" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%"   stopColor={C.brownLt} stopOpacity={0.85}/>
                      <stop offset="100%" stopColor={C.brownLt} stopOpacity={0.25}/>
                    </linearGradient>
                  </defs>
                  ...
                  <Tooltip contentStyle={TT_STYLE} formatter={(v,n)=>[`₹${v} Cr`, n==='demand'?'Demand':'Received']}/>
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{fontSize:11,color:C.text2,paddingTop:8}}/>
                  <Bar dataKey="demand" name="Demand" fill="url(#demandBar)" radius={[8,8,0,0]} barSize={24} animationDuration={1200}/>
            ...
            <GlassCard title="Booking Status" icon={<PieIcon size={15}/>} color={C.brown} delay={0.3}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',alignItems:'center',height:240}}>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={billedPie} cx="50%" cy="50%" innerRadius="58%" outerRadius="82%"
                    ...
                    <Legend verticalAlign="bottom" iconType="circle" iconSize={7} wrapperStyle={{fontSize:10,color:C.text2}}/>
                  </PieChart>
                </ResponsiveContainer>
              </div>
            </GlassCard>
            ...
            <GlassCard title="Tower Collection Rate" icon={<BarChart3 size={15}/>} color={C.blue} delay={0.2}>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={data.tower_list} margin={{top:18,right:10,left:0,bottom:0}}>
                  ...
                  <XAxis dataKey="tower" tick={{fill:C.text3,fontSize:12,fontWeight:600}} axisLine={false} tickLine={false}/>
                  ...
                  <Bar dataKey="collection_rate" name="Rate" radius={[8,8,0,0]} animationDuration={1400}>
            ...
            <GlassCard title="Tower Financial Breakdown" icon={<BarChart3 size={15}/>} color={C.blue} delay={0.5} wide>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={towerFin} margin={{top:20,right:20,left:0,bottom:0}} barGap={3} barCategoryGap="26%">
                  ...
                  <Tooltip contentStyle={TT_STYLE} formatter={v=>`₹${v} Cr`}/>
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{fontSize:11,color:C.text2,paddingTop:6}}/>
                  <Bar dataKey="Demand"      fill={C.blue}    radius={[6,6,0,0]} animationDuration={1200}>
                    <LabelList dataKey="Demand"      position="top" style={{fontSize:10,fontWeight:700,fill:C.blue}}    formatter={v=>`${v}`}/>
                  </Bar>
                  <Bar dataKey="Received"    fill={C.brownLt} radius={[6,6,0,0]} animationDuration={1300}>
                    <LabelList dataKey="Received"    position="top" style={{fontSize:10,fontWeight:700,fill:C.brownDk}} formatter={v=>`${v}`}/>
                  </Bar>
                  <Bar dataKey="Outstanding" fill={C.rose}    radius={[6,6,0,0]} animationDuration={1400}>
        ...
        <div className="tab-content">
          ...
          <div className="charts-grid">
            <GlassCard title="Installment Count by Ageing" icon={<BarChart3 size={15}/>} color={C.blue} delay={0.1}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={ageCountArr} margin={{top:22,right:10,left:0,bottom:0}}>
                  <CartesianGrid strokeDasharray="4 4" stroke="rgba(30,58,95,0.07)" vertical={false}/>
            ...
            <GlassCard title="Outstanding Amount by Ageing" icon={<IndianRupee size={15}/>} color={C.rose} delay={0.2}>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={ageAmtArr} margin={{top:22,right:10,left:0,bottom:0}}>
                  ...
                  <XAxis dataKey="name" tick={{fill:C.text3,fontSize:10,fontWeight:600}} axisLine={false} tickLine={false}/>
                  <YAxis unit=" Cr" tick={{fill:C.text3,fontSize:10}} axisLine={false} tickLine={false}/>
                  <Tooltip contentStyle={TT_STYLE} formatter={v=>`₹${v} Crs`}/>
                  <Bar dataKey="amount" name="Amount" radius={[8,8,0,0]} animationDuration={1400}>
                    {ageAmtArr.map((e,i)=><Cell key={i} fill={e.fill}/>)}
                    <LabelList dataKey="amount" position="top" formatter={v=>`${v} Crs`} style={{fontSize:10,fontWeight:700,fill:C.text}}/>
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </GlassCard>
            <GlassCard title="Ageing Distribution" icon={<PieIcon size={15}/>} color={C.gold} delay={0.3}>
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  ...
                  <Tooltip contentStyle={TT_STYLE} formatter={v=>`₹${v} Crs`}/>
                  <Legend iconType="circle" iconSize={7} wrapperStyle={{fontSize:10,color:C.text2}}/>
                </PieChart>
              </ResponsiveContainer>
            </GlassCard>
          </div>
            ...
            <GlassCard title="Unbilled Milestone Count by Milestone" icon={<BarChart3 size={15}/>} color={C.blue} delay={0.1} wide>
              <ResponsiveContainer width="100%" height={340}>
                <BarChart data={data.top_unbilled_count} layout="vertical" margin={{top:5,right:80,left:8,bottom:5}}>
                  ...
                  <Tooltip contentStyle={TT_STYLE} formatter={v=>v.toLocaleString()}/>
                  <Bar dataKey="unbilled_count" name="Unbilled Count" fill={C.blue} radius={[0,8,8,0]} animationDuration={1400}>
            ...
            <GlassCard title="Unbilled Amount by Milestone" icon={<IndianRupee size={15}/>} color={C.brownLt} delay={0.2} wide>
              <ResponsiveContainer width="100%" height={340}>
                <BarChart data={data.top_unbilled_amount} layout="vertical" margin={{top:5,right:90,left:8,bottom:5}}>
                  ...
                  <YAxis type="category" dataKey="name" width={190} tick={{fill:C.text,fontSize:9,fontWeight:600}} axisLine={false} tickLine={false}/>
                  <Tooltip contentStyle={TT_STYLE} formatter={v=>`₹${v} Crs`}/>
                  <Bar dataKey="unbilled_amount" name="Unbilled Amt" fill={C.brownLt} radius={[0,8,8,0]} animationDuration={1400}>
                    <LabelList dataKey="unbilled_amount" position="right" style={{fontSize:10,fontWeight:700,fill:C.brownDk}} formatter={v=>`${v} Crs`}/>
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </GlassCard>
        ...
        <div className="tab-content">
          <div className="tower-cards">
            {data.tower_list.map((t,i)=>(
              <div key={i} className="tc" style={{'--tc-color':TOWER_COLORS[i%TOWER_COLORS.length], animationDelay:`${i*0.07}s`}}>
          ...
          <div className="charts-grid">
            <GlassCard title="Tower Financial Breakdown" icon={<BarChart3 size={15}/>} color={C.blue} delay={0.2} wide>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={towerFin} margin={{top:22,right:20,left:0,bottom:0}} barGap={3} barCategoryGap="26%">
                  ...
                  <Tooltip contentStyle={TT_STYLE} formatter={v=>`₹${v} Cr`}/>
                  <Legend iconType="circle" iconSize={8} wrapperStyle={{fontSize:11,color:C.text2,paddingTop:6}}/>
                  <Bar dataKey="Demand"      fill={C.blue}    radius={[6,6,0,0]} animationDuration={1200}><LabelList dataKey="Demand"      position="top" style={{fontSize:10,fontWeight:700,fill:C.blue}}    formatter={v=>`${v}`}/></Bar>
                  <Bar dataKey="Received"    fill={C.brownLt} radius={[6,6,0,0]} animationDuration={1300}><LabelList dataKey="Received"    position="top" style={{fontSize:10,fontWeight:700,fill:C.brownDk}} formatter={v=>`${v}`}/></Bar>
                  <Bar dataKey="Outstanding" fill={C.rose}    radius={[6,6,0,0]} animationDuration={1400}><LabelList dataKey="Outstanding" position="top" style={{fontSize:10,fontWeight:700,fill:C.rose}}    formatter={v=>`${v}`}/></Bar>
                </BarChart>
              </ResponsiveContainer>
            </GlassCard>
            <GlassCard title="Outstanding by Tower" icon={<PieIcon size={15}/>} color={C.rose} delay={0.3}>
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie data={data.tower_list} cx="50%" cy="50%" outerRadius={100} innerRadius={45}
